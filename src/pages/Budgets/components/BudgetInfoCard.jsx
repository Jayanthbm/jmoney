...
const BudgetInfoCard = ({
  ...
  const statusText = isOverSpent
    ? isCurrentMonth
      ...
      : "OverSpent: You overspent this period"
    : isCurrentMonth
      ? "In Budget: You are on track"
      : "In Budget: Fantastic! You remained on budget";

  const saved = Math.max(amount - spent, 0);
  ...
  return (
    <>
      <OverviewCard customStyles={{ cursor: "default" }}>
        <div
          className={`budget-info-header ${isOverSpent ? "danger" : "success"}`}
        >
        ...
        <div className="budget-info-grid">
          {isCurrentMonth ? (
            <>
              <div className="info-block hero-block">
            ...
            <>
              <div className="info-block hero-block">
                ...
                <div
                  className={`info-amount ${saved > 0 ? "green-text" : "red-text"}`}
                >
                  ₹{saved.toFixed(2)}
                </div>
              </div>
          ...
          <div className="info-block">
            ...
            <div className="info-amount">
              {percentage_remaining.toFixed(2)}%
            </div>
          </div>
          <div className="info-block">
        ...
        {isCurrentMonth && (
          <div className="remaining-days-text">
            <span className="remaining-days-badge">
              {remainingDays} Remaining day{remainingDays !== 1 ? "s" : ""}
            </span>
          </div>
        )}
        ...
        <ProgressBar
          value={progress}
          color={progress < 101 ? "#3ecf8e" : "#ef4444"}
        />
        <div
          style={{
            display: "flex",
        ...
        >
          <span className="info-subtitle">
            01 {selectedMonth.label} {selectedYear.label}
          </span>
          <span className="info-subtitle">
            {daysInMonth} {selectedMonth.label} {selectedYear.label}
          </span>
        </div>
      </OverviewCard>
      <div
