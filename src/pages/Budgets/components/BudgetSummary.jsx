...
const BudgetSummary = ({ loading, sortedBudgets, refreshData, handleDialogOpen, handleDelete, onBudgetCardClick }) => {
   return (
      <div className="budget-card-container">
         {loading ? (
            <InlineLoader text="Loading Budgets" />
         ) : (sortedBudgets?.length < 1 ? (
            <NoDataCard message="Budgets not loaded, Please Refresh or Create New Budget">
               <div>
                  <Button icon={<IoIosFunnel />} text="Refresh Budgets" variant="primary" onClick={refreshData} />
               </div>
            </NoDataCard>
         ) :
            <>
